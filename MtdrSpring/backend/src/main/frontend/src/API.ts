...
const normalizeAuthUser = (payload: RawAuthEnvelope) => {
    ...
    return {
        ...
        role,
        currentTeamId: base.teamId != null ? String(base.teamId) : (payload.currentTeamId ?? null),
    };
};
 ...
 */
/*
 * Your existing api.ts file
 */
export const _tasksAPI = {
    // Keep your global fallback endpoint
    getAll: (params?: Record<string, string | number>): Promise<AxiosResponse<Task[]>> =>
        api.get("/tasks", {params}),

    // ADD THIS: Fetch tasks specifically isolated to a single team's path context
    getByTeam: (teamId: string | number): Promise<AxiosResponse<Task[]>> =>
...
/*
 * Normalize backend Team shape { teamId, teamName, ... } → frontend Team { id, name }
 */
const normalizeTeam = (raw: unknown): Team => {
    const src = (raw ?? {}) as Record<string, unknown>;
    return {
        id: String(src.id ?? src.teamId ?? ""),
        name: String(src.name ?? src.teamName ?? ""),
        memberCount: typeof src.memberCount === "number" ? src.memberCount : undefined,
    };
};
...
export const teamsAPI = {
    ...
    getMy: async (): Promise<AxiosResponse<Team[]>> => {
        ...
        return makeAxiosResponse(response, raw.map(normalizeTeam));
    },
