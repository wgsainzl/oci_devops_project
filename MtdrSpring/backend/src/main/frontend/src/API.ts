...
export interface SemanticTaskSearchResult {
    taskId: number;
    title: string;
    description: string;
    status: "TODO" | "IN_PROGRESS" | "BLOCKED" | "IN_REVIEW" | "DONE";
    ...
    distance: number;       // The vector similarity score/distance metrics
    contentPreview: string; // The snippet of text matched by the LLM/embedding
}
...
export const tasksAPI = {
    ...
    /*
    * Perform semantic vector search on tasks
    */
    vectorSearch: async (trimmedQuery: string, limit: number = 10): Promise<SemanticTaskSearchResult[]> => {
        const response = await fetch(`${BASE_URL}/ai/tasks/semantic-search`, {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({
                query: trimmedQuery,
