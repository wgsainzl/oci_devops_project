...
export default function NewItem({onCreated, onCancel}: Props): JSX.Element {
    ...
    // load members from OCI using centralized useAPI
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                // Assuming your useAPI wrapper has a users/members lookup endpoint similar to below
                // If it's located elsewhere, adjust to your exact schema (e.g., useAPI.users.getAll())
                ...
                setError("Unable to connect to developers user catalog.")
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Payload matching NewTaskPayload shape expected by tasksAPI.create
            const payload = {
            ...
            console.log("Inserting a real task linked to a real user via centralized API:", payload)

            // FIXED: Now utilizing your centralized tasks api wrapper
            const newTask = await useAPI.tasks.create(payload)
            ...
            // Handle the custom 409 database sync conflict message gracefully if caught
            if (err.message?.includes("409")) {
                setError("Conflict (409): Verify that Sprint ID 1 exists in your Oracle physical table.")
            } else {
